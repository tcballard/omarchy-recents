...
Item {
    ...
    function requestAction(kind) {
        ...
        actionProcess.command=["/usr/bin/timeout","20s",decodeURIComponent(Qt.resolvedUrl("bin/recents-act").toString().replace(/^file:\/\//,"")),kind,actionPath]
    ...
    function checkVisible() {
        ...
        checkProcess.command=["/usr/bin/timeout","5s",helper,"check"]
    ...
    Ui.KeyboardPanel {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Text { textFormat: Text.PlainText; text:"Recents"; color:root.foreground; font.family:Style.font.family; font.pixelSize:Style.font.heading; font.bold:true }
                Item { Layout.fillWidth:true }
                Text { textFormat: Text.PlainText; text:root.service && root.service.refreshing ? "Updating…" : root.rows.length+" files"; color:root.foreground; opacity:.6; font.pixelSize:Style.font.caption }
                Text { textFormat: Text.PlainText; text:"×"; color:root.foreground; font.pixelSize:Style.font.heading; MouseArea {anchors.fill:parent;onClicked:root.close()} }
            }
            Rectangle {
                ...
                TextInput {
                    ...
                    Text { textFormat: Text.PlainText; visible:!search.text; text:"Search file name or folder"; color:root.foreground; opacity:.45; font:search.font }
                }
            }
            Flow {
                ...
                Repeater {
                    ...
                    Rectangle {
                        ...
                        Text { textFormat: Text.PlainText; id:label; anchors.centerIn:parent; text:modelData; color:index===root.typeIndex ? Color.popups.background : root.foreground; font.pixelSize:Style.font.bodySmall; font.family:Style.font.family }
            ...
            RowLayout {
                Layout.fillWidth:true
                Text { textFormat: Text.PlainText; text:"Source: "+root.sources[root.sourceIndex]+" ▾"; color:root.foreground; font.pixelSize:Style.font.caption; MouseArea {anchors.fill:parent;onClicked:root.sourceIndex=(root.sourceIndex+1)%root.sources.length} }
                Item { Layout.fillWidth:true }
                Text { textFormat: Text.PlainText; text:search.activeFocus ? "↓ browse · Tab type" : "j/k move · / search · s source"; color:root.foreground; opacity:.55; font.pixelSize:Style.font.caption }
            }
            ListView {
                ...
                delegate:Column {
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            Text { textFormat: Text.PlainText; text:Model.glyph(modelData.kind,!(root.service && root.service.glyphsSupported) || !!root.service.config.plainGlyphs); color:Color.accent; font.family:Style.font.family; font.pixelSize:Style.font.body }
                            ColumnLayout {
                                Layout.fillWidth:true; spacing:Style.space(3)
                                Text { textFormat: Text.PlainText; objectName:"recent-file-name"; Layout.fillWidth:true; text:Model.name(modelData.path); color:root.foreground; font.family:Style.font.family; font.pixelSize:Style.font.body; elide:Text.ElideMiddle }
                                Text { textFormat: Text.PlainText; objectName:"recent-file-parent"; Layout.fillWidth:true; text:Model.parent(modelData.path,root.service ? root.service.home : ""); color:root.foreground; opacity:.55; font.family:Style.font.family; font.pixelSize:Style.font.caption; elide:Text.ElideMiddle }
                            }
                            ColumnLayout {
                                Text { textFormat: Text.PlainText; Layout.alignment:Qt.AlignRight; text:Qt.formatDateTime(new Date(modelData.ts*1000),"HH:mm"); color:root.foreground; font.pixelSize:Style.font.bodySmall }
                                Text { textFormat: Text.PlainText; text:modelData.sources.join(" · "); color:root.foreground; opacity:.55; font.pixelSize:Style.font.caption }
                            }
                        }
            ...
            Text { textFormat: Text.PlainText; objectName:"recent-detail"; Layout.fillWidth:true; visible:!!root.detail; text:root.detail; elide:Text.ElideMiddle; color:root.foreground; font.pixelSize:Style.font.caption }
            Text { textFormat: Text.PlainText;
                objectName:"recent-message"
            ...
            Text { textFormat: Text.PlainText; visible:!!(root.service && root.service.capped); text:"Showing most recent 2,000 · search covers this index"; color:root.foreground; font.pixelSize:Style.font.caption }
            Flow {
